...
:heavy_check_mark: 2) 앱이 close()·closesocket() 등을 호출하지 않으면 어떻게 되나?
:heavy_check_mark: 커널 메모리 누수(leak) 발생
...
:heavy_check_mark: 3) 프로세스가 정상 종료되면?
...
즉:
:star: 프로세스 종료 시점에서는 커널 메모리 누수는 사라진다.
하지만…

:heavy_check_mark: 4) 프로세스가 실행 중인 동안 close 누락은 치명적이다
특히 서버 프로세스의 경우:
...
:heavy_check_mark: 5) 예외: close를 하지 않아도 누수가 아닌 경우?
거의 없음. 다만 특수 케이스로:
