URL Encode(Percent-Encoding)은 URI에서 허용되지 않는 문자나 특별한 의미를 가지는 문자를 안전하게 전달하기 위해 특정 형식으로 변환하는 과정입니다.
아래에 핵심을 정리하였습니다.

:heavy_check_mark: URL Encoding이 필요한 이유
...
:heavy_check_mark: URL Encoding 방식
